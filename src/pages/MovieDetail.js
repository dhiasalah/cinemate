...
export const MovieDetail = () => {
  ...
  useEffect(()=>{
    async function fetchMovie(){
      const response=await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=b8283688e06693b6d43cc65b1d59eda1`);
    ...
    <main>
      <section className="flex justify-around flex-wrap py-5">
        ...
        <div className="max-w-2xl text-gray-700 text-lg dark:text-white ">
          <h1 className="text-4xl p-2 m-2 text-center lg:text-left">{movie.title}</h1>
          <p className="my-4 mx-2">{movie.overview}</p>
          {movie.genres?(
            <p className="my-7 mx-2 flex flex-wrap garp-2">
          ...
          <div className=" mx-2 flex items-center">
              ...
              <span href="#" className="text-sm font-medium text-gray-900  dark:text-white">{movie.popularity} reviews</span>
          </div>
